        ...
        public static void Load(TileDataLoader tileData)
        {
            ...
            if (!File.Exists(ignoreCot))
            {
                ...
                ushort[] ignoreCoTTiles =
                {
                    ...
                    0x0CD0, 0x0CD3, 0x0CD6, 0x0CD8, 0x0CDA, 0x0CDD, 0x0CE0, 0x0CE3, 0x0CE6, 0x0CFB,
                ...
                {
                    writer.WriteLine(ignoreCoTTiles[i]);
                }
            }

            TextFileParser ignoreCoTParser = new TextFileParser(
                File.ReadAllText(ignoreCot),
                new[] { ' ', '\t', ',' },
                new[] { '#', ';' },
                new[] { '"', '"' }
            );
            ...
            while (!ignoreCoTParser.IsEOF())
            {
                ...
                if (ss != null && ss.Count != 0 && ushort.TryParse(ss[0], out ushort graphic))
                {
