...
namespace ClassicUO.Game
{
    internal static class GameActions
    {
        ...
        // Dust765: flag de casting (usado para bloquear movimento)
        ...
        public static void CastSpellFromBook(int index, uint bookSerial)
        {
            if (index >= 0)
            {
                ...
                if (player?.OnCasting != null && !iscasting && ProfileManager.CurrentProfile?.OnCastingGump == true)
                {
                    player.OnCasting.Start((uint)index, 0);
                }
