...
namespace ClassicUO.Game.Map
{
    ...
    internal sealed class ChunkMesh
    {
        ...
        /// <summary>
        /// Bumped when a profile option changes what a mesh is allowed to contain. Chunks bake
        /// those exclusions at build time, so without this every chunk that is not otherwise
        /// touched would keep rendering with the old setting until an object moves in it.
        /// </summary>
        public static int InvalidationVersion;

        private int _builtVersion = -1;
        private bool _animatedWaterEffect;

        public bool NeedsRebuild => IsDirty || _builtVersion != InvalidationVersion;
        ...
        public void Build(Chunk chunk, World world, GraphicsDevice graphicsDevice)
        {
            ...
            Statics.Reset();
            IsDirty = false;
            _builtVersion = InvalidationVersion;
