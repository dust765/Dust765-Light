...
namespace ClassicUO.Network
{
    sealed class PacketHandlers
    {
        ...
        private static void OpenContainer(World world, ref StackDataReader p)
        {
            ...
            else
            {
                ...
                if (item != null)
                {
                    ...
                    if (ProfileManager.CurrentProfile?.GridContainerEnabled == true)
                    {
                        GridContainer existingGrid = UIManager.GetGump<GridContainer>(serial);
                        if (existingGrid != null)
                        {
                            existingGrid.InvalidateContents = true;
                        }
                        else
                        {
                            UIManager.Add(new GridContainer(world, serial, graphic) { InvalidateContents = true });
                        }
                    }
                    else
                    {
        ...
        private static void NewCharacterAnimation(World world, ref StackDataReader p)
        {
            ...
            if (mobile == world.Player)
            {
                UOClassicCombatSwingGump.NotifyPlayerNewAnimation(world);
            }
        }
